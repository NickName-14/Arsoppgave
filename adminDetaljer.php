...
<?php
 ...
 $sql = "SELECT ProduktiBestilingId, Totalpris FROM Bestilinger WHERE Kunde = '" . $_SESSION['id'] . "'";

$result_outer = $link->query($sql);

if ($result_outer->num_rows > 0) {
    while ($row_outer = $result_outer->fetch_assoc()) {
        $produkt_handlevognid = $row_outer['ProduktiBestilingId'];
        echo "<h1>Produkter i bestiling nummer: ". $row_outer['ProduktiBestilingId'] ."</h1>";
